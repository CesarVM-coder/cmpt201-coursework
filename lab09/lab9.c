...
port number). Answer: The client is trying to connect to the server with IP
address 127.0.0.1, and port 8000.
2. Is it UDP or TCP? How do you know?
This program uses TCP. We can tell because the client calls connect().
TCP needs connection before sending data, unlike UDP that is connectionless.
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? Answer: The client get this data feom
the std input, as shown in the code where the command read(STDIN_FILENO, buf,
BUF_SIZE) is used to read the user input.
4. How does the client program end? How can you tell that in the code?
Answer: The program naturally ends when stdin finishes reading the user input,
and the loop exits. Other erros may cause the program to finish early via
handle_error().
*/
...
void *handle_client(void *arg) {
  ...
  while ((num_read = read(cfd, buf, BUF_SIZE)) > 0) {
    // Print cliend ID and message
    buf[num_read] = '\0';

    pthread_mutex_lock(&count_mutex);
    int temp_counter = ++total_message_count;
    pthread_mutex_unlock(&count_mutex);
    printf("Msg #%4d; Client ID %d: %s", temp_counter, client_id, buf);
  }
...
int main() {
  ...
  for (;;) {
    ...
    client_inf->client_id = client_id_counter++;
    pthread_mutex_unlock(&client_id_mutex);
    pthread_t tid;

    if (pthread_create(&tid, NULL, handle_client, client_inf) != 0) {
