...
ANS: The client is sending data through sockets with the TCD protocol.
2. What data is the client sending to the server?
The client is sending 5 string messages as shown {"Hello", "Apple", "Car",
"Green", "Dog"}.
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
run_acceptor() argument is acceptor_argss struct,  This struct contains a flag
for the acceptor loop, a pointer to the list of messages and a mutex for the
syncronization of the list.
2. How are received messages stored?
The messages are stores in a linked list. Each message is copied into a node
with help of mutexes for safety.
3. What does `main()` do with the received messages?
main() waits until all expeceted messages arrive, and then writes the messages
and frees the memory.
4. How are threads used in this sample code?
There purpose of the threads in this code are the following. One of them is an
acceptor thread, whose job is to accept new connections. There are multiple
client threads(one per message) to read the messages.
*/
