...
int main() {
  ...
  if (pub_file == NULL) {
    handle_error("ERROR: Open public key");
  }
  ...
  if (pubkey == NULL) {
    handle_error("ERROR: Read public key");
  }
...
#define MAX_FILE_SIZE 512
  ...
  if (EVP_DigestVerifyInit(context, NULL, EVP_sha256(), NULL, pubkey) != 1) {
    EVP_MD_CTX_free(context);
    return -1;
  }

  if (EVP_DigestVerifyUpdate(context, message, message_len) != 1) {
    EVP_MD_CTX_free(context);
    return -1;
  }

  int status = EVP_DigestVerifyFinal(context, signature, sig_len);
